...
def local_sgd_od(
    ...
    # [CRITICAL FIX] Trả lại AdamW cho FL. Trước đây từng thử đổi sang SGD vì sợ nổ Loss,
    # nhưng SGD với LR=5e-4 gần như làm biến mất update (Vanishing updates), khiến 
    # mô hình HFL bị dậm chân tại chỗ.
    # Các lỗi nổ Loss trước đây thực ra là do Ultralytics default warmup_bias_lr=0.1 (đã fix ở dưới).
    opt_choice = optimizer_name or 'AdamW'
    print(f"[DiffLR] Sử dụng Optimizer {opt_choice} (lr={lr:.4f}).")
    try:
        from config.settings import fed_cfg
    ...
    if global_c is not None and local_c is not None:
        ...
        for k in state_before:
            if k in state_after and k in global_c and k in local_c:
                if (
                    not torch.is_tensor(state_before[k])
                    or not torch.is_tensor(state_after[k])
                    or not torch.is_floating_point(state_before[k])
                    or not torch.is_floating_point(state_after[k])
                ):
                    continue
...
def evaluate_od(student_model, test_yaml: str, device: str = "cpu") -> dict:
    ...
    def _results_metric(*names: str):
        result_dict = getattr(results, 'results_dict', {}) or {}
        for name in names:
            if name in result_dict:
                try:
                    return float(result_dict[name])
                except (TypeError, ValueError):
                    return None
        return None

    val_box_loss = _results_metric('val/box_loss', 'val/box_loss(B)')
    val_cls_loss = _results_metric('val/cls_loss', 'val/cls_loss(B)')
    val_dfl_loss = _results_metric('val/dfl_loss', 'val/dfl_loss(B)')
