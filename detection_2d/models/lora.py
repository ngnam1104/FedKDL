...
def inject_lora(module: nn.Module,
    ...
    - strategy = "fixed"/"all": Chèn mọi layer mục tiêu với cùng rank
    ...
    for name, sub_module in targets:
        ...
        if any(cls_name in {"Detect", "v10Detect", "Segment", "Pose", "OBB"} for cls_name in path_classes):
            continue
        try:
            head_idx = len(module.model) - 1
        except Exception:
            head_idx = None
        if head_idx is not None and layer_idx == head_idx:
            continue
        ...
        if is_target:
            ...
            if strategy == "neck_head_only":
                ...
                backbone_rank = int(os.getenv("FEDKDL_LORA_BACKBONE_RANK", max(1, rank // 2)))
                neck_rank = int(os.getenv("FEDKDL_LORA_NECK_RANK", rank))
                if layer_idx != -1:
                    if layer_idx < 10:
                        current_rank = backbone_rank
                    else:
                        current_rank = neck_rank
            elif strategy not in ("fixed", "all"):
                raise ValueError(f"Unknown LoRA injection strategy: {strategy}")
