...
def _robust_clip_range(x_flat: torch.Tensor) -> tuple[float, float]:
    """Return a percentile-clipped range for INT8 scale calibration."""
    from config.settings import fed_cfg

    percentile = float(getattr(fed_cfg, 'INT8_CLIP_PERCENTILE', 100.0))
    min_numel = int(getattr(fed_cfg, 'INT8_CLIP_MIN_NUMEL', 256))
    x_min = x_flat.min().item()
    x_max = x_flat.max().item()
    if percentile >= 100.0 or x_flat.numel() < min_numel:
        return x_min, x_max
    ...
    # FedKDL sends INT8 deltas by default. Symmetric clipping keeps zero
    # centered while preventing rare outliers from wasting most quant levels.
    q = torch.quantile(abs_flat, percentile / 100.0).item()
    clip_abs = max(float(q), 1e-12)
    if clip_abs >= max_abs:
        return x_min, x_max
    return -clip_abs, clip_abs
...
def quantize_tensor(x: torch.Tensor, bits: int | None = None) -> QuantizedTensor:
    ...
    x_min, x_max = _robust_clip_range(x_flat)
    ...
    # Lượng tử hóa
    x_to_quantize = torch.clamp(x_flat, min=x_min, max=x_max)
    x_quantized = torch.round(x_to_quantize / scale).long() + zero_point
...
def compute_payload_bits(
    ...
    Components:
        ...
        - Indices (int32):  K × 32 bits  (dùng int32 cố định)
    ...
    """
    K = len(topk_indices)
    value_bits = K * 8           # INT8: 1 byte/value
    idx_bits   = K * 32          # index dùng int32 cố định
...
class SparseINT8Payload:
    ...
    def __init__(
        ...
        total_params: int,
        shapes: List[Tuple[str, torch.Size, int]] | None = None,
    ):
...
def pack_delta_payload(
    ...
    # Handle asymmetric uplink/downlink: state_dict might have fewer keys than reference_state
    if not set(state_dict).issubset(set(reference_state)):
        extra = sorted(set(state_dict) - set(reference_state))
        raise ValueError(
            f"Delta payload keys must be a subset of the reference state; extra={extra[:5]}"
        )
    delta_state = {
...
def unpack_delta_payload(
    ...
    """Reconstruct a state from an INT8 update and its exact link reference template."""
    decoded_delta = unpack_payload(payload, template)
    return {
        key: template[key].cpu() + decoded_delta[key]
        for key in template
    }
