...
from kdl_core.models.autoencoder import get_model_state_dict_copy
from kdl_core.algorithms.local_trainer import local_sgd
...
from kdl_core.knowledge_compression.int8_quantization import SparseINT8Payload
from kdl_core.algorithms.aggregator import fedavg_intra_cluster, fedavg_global
...
class SensorWorker:
    ...
    def train_and_compress(
        ...
        mu: float = 0.0,
        device: str = 'cpu',
    ) -> Optional[Tuple[SparseINT8Payload, float]]:
        """
        Huấn luyện và nén gửi lên Fog.
        Nếu pin <= 0, trả về None.
        Returns: (payload, loss)
        """
        if not self.alive:
            return None
        ...
        self.drift_monitor.update(self.sensor_id, avg_loss)
