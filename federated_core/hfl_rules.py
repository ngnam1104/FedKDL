...
def blend_state_dicts(
    ...
    """
    Blend relay states as alpha * self + (1-alpha) * neighbor.

    LoRA factors are blended through the same SVD-LoRA aggregator used for
    intra/global aggregation, i.e. it blends B @ A and refactorizes once.
    This avoids maintaining a second, subtly different LoRA SVD path.
    """
    ...
    return svd_lora_aggregate([self_sd, neighbor_sd], [alpha, 1.0 - alpha])
