...
class BaseGateway:
    ...
    def __init__(self, initial_state):
        ...
        # Separate optimizer caches for Gateway Proxy-FT and KD — must not be mixed
        # because CustomDetectionTrainer and KDDetectionTrainer have different param groups.
        self.proxy_ft_optimizer_state = None
        self.kd_optimizer_state = None
        self.pure_aggregated_state = None  # [WiSE-FT] Lưu FedAvg thuần trước server_mix
    ...
    def aggregate_global(
        ...
        for relay_id, state in relay_final_states.items():
            if isinstance(state, dict) and '__scaffold_delta_c__' in state:
                ...
                scaffold_client_counts.append(max(int(state.get('__scaffold_client_count__', 1)), 1))
                state = {
                    key: value
        ...
        if states_list:
            ...
            # [WiSE-FT] Lưu kết quả FedAvg thuần trước khi server_mix
            # để Proxy FT có thể blend một cách sạch sẽ sau này
            import copy as _copy
            self.pure_aggregated_state = _copy.deepcopy(aggregated_state)
        ...
        if delta_c_list:
            ...
            if total_clients > 0:
                ...
                weights = [1.0 / len(delta_c_list)] * len(delta_c_list)
            delta_c_agg = {}
